...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        ...
        $stmt = $pdo->prepare('INSERT INTO usuarios (nome, usuario, email, senha, funcao_id, oculto) VALUES (?, ?, ?, ?, ?, ?)');
        $ok = $stmt->execute([$nome, $usuario, $email, $senha_hash, $funcao_id, $oculto]);
        if ($ok) {
            $sucesso = true;
        } else {
        ...
        <div class="form-container">
            ...
            <form method="post" autocomplete="off">
                <div class="form-group">
                    ...
                    <input type="text" name="nome" id="nome" class="form-control" required value="<?= isset($_POST['nome']) ? htmlspecialchars($_POST['nome']) : '' ?>">
                </div>
                <div class="form-group">
                    <label for="usuario">Usuário</label>
                    <input type="text" name="usuario" id="usuario" class="form-control" required value="<?= isset($_POST['usuario']) ? htmlspecialchars($_POST['usuario']) : '' ?>">
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <select name="funcao_id" id="funcao_id" class="form-control" required>
                        ...
                        <?php foreach ($funcoes as $f): ?>
                            <option value="<?= $f['id'] ?>" <?= (isset($_POST['funcao_id']) && $_POST['funcao_id'] == $f['id']) ? 'selected' : '' ?>><?= htmlspecialchars($f['nome']) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group form-check">
                    <input type="checkbox" name="oculto" id="oculto" class="form-check-input" <?= (isset($_POST['oculto']) ? 'checked' : '') ?>>
